...
// Song colors chosen to match each song's theme/mood
const songs = [
  {
    id: 1,
    title: "God Morgen",
    emoji: "🌞",
    // Warm sunrise orange-yellow — morgenlys og solskin
    color: "#FFB830",
    glow: "#FFB83055",
    cardBg: "linear-gradient(135deg, #3D2E7C 60%, #5C3D00 100%)",
    verses: [
      "God morgen, god morgen, [navn] er vågen nu,\nSolen skinner klart og fuglene synger ku!\nHop op af sengen, [navn], en ny dag er her,\nGod morgen, god morgen, vi er så glade her! 🌸",
      "Stræk dine arme, [navn], og gab en gang stor,\nDagen er begyndt nu ude på vor jord!\nVask dine hænder, [navn], og spis din morgenmad,\nGod morgen, god morgen – i dag bli'r vi glad! 🥣"
    ]
  },
  {
    id: 2,
    title: "Lille Stjerne",
    emoji: "⭐",
    // Deep midnight blue-violet — nattehimmel med stjerner
    color: "#A8CFFF",
    glow: "#A8CFFF44",
  ...
  },
  {
    ...
    title: "Hop og Dans",
    emoji: "🐸",
    // Lively grass green — hoppe og danse udenfor
    color: "#5DDA6A",
    glow: "#5DDA6A44",
  ...
  },
  {
    ...
    title: "Regnvejrssang",
    emoji: "🌧️",
    // Sky blue — regnvejr og vandpytter
    color: "#60C4F0",
    glow: "#60C4F044",
  ...
  },
  {
    ...
    title: "Godnat Sang",
    emoji: "🌙",
    // Soft lavender — søvn, ro og drømme
    color: "#D4AAFF",
    glow: "#D4AAFF44",
...
// Faithful recreation of the Ohna elephant illustration from the image:
// Grey elephant sitting, playing guitar, drum to the right, gold stars & music notes
function OhnaElephant({ size = 220 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 340 320" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      {/* ── TRUNK (curled slightly left, open mouth = singing) ── */}
      <path d="M138 198 Q118 225 122 255 Q124 268 133 266 Q142 264 140 251 Q136 232 150 210" 
            fill="#9AAAB8" stroke="#8A9BAA" strokeWidth="1"/>
      {/* trunk tip */}
        ...
        <div style={{ display:"flex", gap:8, flexWrap:"wrap", justifyContent:"center", marginBottom:22 }}>
          {songs.map(s => (
            <button key={s.id} className="song-pill"
              onClick={() => { setSelected(s); setVerse(0); }}
              style={{
